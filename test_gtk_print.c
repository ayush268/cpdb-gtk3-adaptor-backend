/**This is just testing out the high level printing API provided by GTK 
 * and I am not claiming ownership of this code 
 * **/
...
int main(int argc, char **argv)
{
    ...
    gtk_window_set_title(w->window, "Printing");
    gtk_container_set_border_width(w->window, 10);

    g_signal_connect(w->window, "destroy", G_CALLBACK(gtk_main_quit), NULL);

    w->chooser = gtk_file_chooser_button_new("Select File", GTK_FILE_CHOOSER_ACTION_OPEN);

    gtk_file_chooser_set_current_folder(w->chooser, g_get_home_dir());

    print = gtk_button_new_from_stock(GTK_STOCK_PRINT);
    g_signal_connect(print, "clicked", G_CALLBACK(print_file), w);

    hbox = gtk_hbox_new(FALSE, 5);
    gtk_box_pack_start(hbox, w->chooser, FALSE, FALSE, 0);
    gtk_box_pack_start(hbox, print, FALSE, FALSE, 0);

    gtk_container_add(w->window, hbox);
...
/*begin the printing by retreiving the contents of the 
selected files and
spiltting it into single lines of text*/

static void
begin_print(GtkPrintOperation *op,
            ...
            Widgets *w)
{
    ...
    /*retrieve the file contents and 
    split it into lines of text**/
    ...
    /** Based on the height of the page and the font size,
    ...
    gtk_print_operation_set_n_pages(op, w->data->total_pages);
    g_free(contents);
}
